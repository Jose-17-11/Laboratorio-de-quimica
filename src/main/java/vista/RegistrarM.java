	...
	public RegistrarM(String usuario) {
		...
		registrarEntrada.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				RegistrarAccesoMaestro registrar = new RegistrarAccesoMaestro();
				boolean verificar = registrar.acceso(matriculaM, salon);
				if(verificar) {
					try {
						registrar.registro(matriculaM, salon, grupo, materia, carrera);
						exitosamente.setText("Se registro acceso exitosamente.");
					} catch (SQLIntegrityConstraintViolationException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
					}
				}else {
					exitosamente.setText("No tiene laboratorio asignado en esta hora.");
				}
			}
		});
	}
